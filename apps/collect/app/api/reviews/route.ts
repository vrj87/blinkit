...
import { z } from "zod";
import { appendReviews, getStatus, persistNormalized } from "@/lib/store";
import type { RawReview, ReviewSource } from "@/lib/types";

const ReviewSchema = z.object({
  source: z.enum(["app_store", "play_store", "reddit", "forum", "social", "product_review", "web_ui"]),
...
export async function POST(request: NextRequest) {
  ...
  if (body.mode === "normalize") {
    ...
    return NextResponse.json({ ok: true, message: "Normalized → data/discovery/", ...result });
  }
  if (body.mode === "bulk") {
